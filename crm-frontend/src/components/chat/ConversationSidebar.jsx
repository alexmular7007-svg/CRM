...
const ConversationSidebar = ({ rooms, currentRoom, onSelectRoom, onCreateRoom, isCompactMobile = false }) => {
  ...
  return (
    <div className="w-full h-full bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col overflow-hidden">
      {/* Header - Only show on desktop, hide on mobile compact mode */}
      {!isCompactMobile && (
      ...
      {/* Mobile compact header - Show search bar only */}
      {isCompactMobile && (
      ...
      {/* Conversation List - Proper spacing and touch targets */}
            ...
            {filteredRooms.map((room) => {
              ...
              return (
                <motion.div
                  ...
                  className={`px-3 sm:px-4 py-4 cursor-pointer transition-colors touch-target min-h-[72px] flex items-center ${
