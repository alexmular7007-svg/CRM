...
export default function EmailCampaignForm({ campaign, onSuccess }) {
  ...
  const { data: templatesResponse, isLoading: templatesLoading, error: templatesError } = useQuery({ queryKey: ['email-templates', currentWorkspace?.id], queryFn: () => emailCampaignService.listTemplates(currentWorkspace.id), enabled: !!currentWorkspace?.id })
  const { data: segmentsResponse, isLoading: segmentsLoading, error: segmentsError } = useQuery({ queryKey: ['email-segments', currentWorkspace?.id], queryFn: () => emailCampaignService.listSegments(currentWorkspace.id), enabled: !!currentWorkspace?.id })
  ...
  return <form onSubmit={submit} className="space-y-4">
    ...
    <div><label htmlFor="campaign-template" className="text-sm font-medium text-gray-700 dark:text-gray-300">Template *</label><select id="campaign-template" name="templateId" value={form.templateId} onChange={change} disabled={templatesLoading} className={fieldClass}><option value="">{templatesLoading ? 'Loading templates...' : templates.length === 0 ? 'No templates available' : 'Select a template'}</option>{templates.map((template) => <option key={template.id} value={template.id}>{template.name}</option>)}</select>{templatesError && <p className="mt-1 text-xs text-red-600">Failed to load templates</p>}{error('templateId')}</div>
    ...
    {form.recipientMode === 'segment' ? <div><label htmlFor="campaign-segment" className="text-sm font-medium text-gray-700 dark:text-gray-300">Recipient segment</label><select id="campaign-segment" name="segmentId" value={form.segmentId} onChange={change} disabled={segmentsLoading} className={fieldClass}><option value="">{segmentsLoading ? 'Loading segments...' : segments.length === 0 ? 'No segments available' : 'Select a segment'}</option>{segments.map((segment) => <option key={segment.id} value={segment.id}>{segment.name} ({segment.leadCount || 0})</option>)}</select>{segmentsError && <p className="mt-1 text-xs text-red-600">Failed to load segments</p>}{error('segmentId')}</div> : <div><label htmlFor="campaign-recipients" className="text-sm font-medium text-gray-700 dark:text-gray-300">Emails</label><textarea id="campaign-recipients" name="recipients" value={form.recipients} onChange={change} placeholder="name@example.com, another@example.com" rows="3" className={fieldClass} />{error('recipients')}</div>}
